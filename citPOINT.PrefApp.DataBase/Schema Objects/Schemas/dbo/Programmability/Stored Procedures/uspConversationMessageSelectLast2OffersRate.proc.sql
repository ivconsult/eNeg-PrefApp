CREATE PROCEDURE [dbo].[uspConversationMessageSelectLast2OffersRate]
...
(
Select  percentage,RatedDate From (
       --Sent Messages [My Messages Own]
	  SELECT top(@OwnTop) ConversationMessage.percentage,
	  ...
	  Select  percentage,RatedDate From (
	  SELECT top (@CounterPartTop) ConversationMessage.percentage,
								   ConversationMessage.RatedDate 
	  FROM   ConversationMessage 
			 INNER JOIN NegConversation 
			   ON ConversationMessage.NegConversationID = 
